...
public class Car extends Group {
	...
	public Car(Path path) {
		...
		transition.setDuration(Duration.seconds(calculateLength(path) / carSpeed)); // Calculates duration based on path length and car speed
		transition.setInterpolator(Interpolator.LINEAR);
		transition.setCycleCount(1);
		transition.play();
		
		// Updates the current position of the car during the transition by converting its local coordinates to scene coordinates.
	...
	public double calculateLength(Path path) {
		...
		for (PathElement element : elements) {
			...
			if (element instanceof MoveTo) {
				...
				//If the element is a LineTo, calculates the length of the line segment and adds it to the total length.
	...
	//Checks if this car is colliding with another car.Calculates the distance between the two cars using the distance formula.
	...
	public boolean isOnPath(Car otherCar) {
		...
		// Checks for intersection between the shape of the other car and the path of this car(CurrentCar).
